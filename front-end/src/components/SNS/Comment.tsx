  ...
  return (
    <>
      <s.Container $isdelete={isDelete} >
        ...
        <s.CommentArea $ismodify={isModify}>
          <s.CommentAuthorArea>
            ...
            { isUserComment === true ? (
              <s.CommentMenu onClick={modonclick}>수정</s.CommentMenu>
            ) : (
              <></>)}
            { isUserComment === true ? (
      ...
      <s.Modify $ismodify={isModify} ref={modifyRef}>
        ...
        <s.ModifyButton><Button width="64px" height="40px" size="14px" bold="500" onClick={modifyOnSubmit}>수정</Button></s.ModifyButton>
      </s.Modify>
      </s.Container>
    </>
  )
