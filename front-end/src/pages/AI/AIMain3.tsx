...
const AIMain3: React.FC = () => {
  ...
  const [totalCount, setTotalCount] = useState<number>(5); // 총 운동 횟수를 저장하는 상태
  ...
  // 컴포넌트가 처음 렌더링될 때 모델을 로드하는 useEffect
  // useEffect(() => {
  ...
  const init = async () => {
    ...
    const loop = async () => {
      ...
      if (context) {
        ...
        console.log('Drawing on canvas with pose');
      }
  ...
  useEffect(() => {
    if (correctCount + inCorrectCount > 0) {
      setResults((prevResults) => [...prevResults, { set: correctCount, isError: false, message: '올바른 자세' }]);
    }
    ...
    <s.Container>
      ...
      <s.AIArea>
        ...
        <s.PageBody>
          ...
          <s.SettingBtn onClick={() => setIsSettingsModalOpen(true)}>운동 설정</s.SettingBtn>
          {/* 카메라 시작 버튼 */}
          ...
          {/* 카메라 종료 버튼 */}
          {selectedExercise && cameraStarted && <s.SettingBtn onClick={stop} children="카메라 종료" />}
          {selectedExercise && (
            <s.ProgressBar>
